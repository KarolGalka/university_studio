...
import json
import databaseTest
from data_algorithms import twitter
...
def get_best_categories(all_categories):
    ...
    best_categories = [category for i, category in enumerate(sorted_categories) if i < 3]
...
def get_fitting_tweets_categories(username):
    last_tweets = twitter.find_user_tweets(username)
    all_categories = list()
    for tweet in last_tweets:
        if not twitter.is_tweet_valuable(tweet):
            continue
