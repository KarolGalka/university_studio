PLACES = [
    {
      "name" : "Muzeum Narodowe w Krakowie",
      "category" : "art and entertainment"
    },
    {
      "name" : "Kraków Główny",
      "category" : "travel"
    },
    {
      "name" : "Krakowski Park Technologiczny Sp. z o.o. (KPT)",
      "category" : "technology and computing"
    },
    {
      "name" : "Maxi Moda Kraków",
      "category" : "style and fashion"
    },
    {
      "name" : "Serwis i Salon Porsche Jaguar Kraków",
      "category" : "automotive and vehicles"
    },
    {
      "name" : "Rynek Główny",
      "category" : "society"
    },
    {
      "name" : "Akademia Górniczo-Hutnicza im. Stanisława Staszica w Krakowie",
      "category" : "education"
    },
    {
      "name" : "Żłobek nr 1 Samorządowy",
      "category" : "family and parenting"
    },
    {
      "name" : "Radio Kraków S.A",
      "category" : "news"
    },
    {
      "name" : "Stadion Miejski im. Henryka Reymana",
      "category" : "sports"
    },
    {
      "name" : "Biblioteka Główna Akademii Górniczo-Hutniczej",
      "category" : "science"
    },
    {
      "name" : "My Fitness Place Kraków | Siłownia | Basen | Klub Fitness | Bonarka",
      "category" : "health and fitness"
    },
    {
      "name" : "Ogród Zoologiczny w Krakowie",
      "category" : "pets"
    }
  ]
